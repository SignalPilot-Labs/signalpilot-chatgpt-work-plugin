...
- The download in Step 3 is a shell command. The Codex sandbox blocks network access by
  default. If `curl` fails with a network error, request approval to run the command
  outside the sandbox. Or tell the user to set `network_access = true` under
  `[sandbox_workspace_write]` in `~/.codex/config.toml`.
- Get the links only when you are ready to download. Do not get links before an approval
  prompt. They expire in two minutes.
